// New API file to force browser reload
...
// Get current schedule for a project - NEW VERSION
export const getCurrentScheduleNew = async (projectId: string): Promise<ProjectSchedule | null> => {
  console.log('🔍 getCurrentScheduleNew called - BRAND NEW FILE - VERSION 7 - FORCE RELOAD');
  const response = await fetch(`/api/projects/${projectId}/schedules/current?t=${Date.now()}&v=7&force=true&reload=true&new=true`, {
    credentials: 'include',
    cache: 'no-cache',
    headers: {
